...
// Instant re-render when background writes new exchange or usage data to storage
chrome.storage.onChanged.addListener((changes, area) => {
  if (area !== 'local') return;
  const watched = ['todayStats', 'allTimeStats', 'lastUsageSnapshot', 'lastModel', 'lastAdaptive'];
  if (watched.some(k => k in changes)) renderStats();
});

// Keep the 30s fallback for anything storage.onChanged misses (e.g. tier updates)
